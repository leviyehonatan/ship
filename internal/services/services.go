...
func Ensure(client *shipssh.Client, cfg *config.ShipConfig, appName string, isLocal bool) (map[string]string, error) {
	...
	for name, svc := range cfg.Services {
		...
		// Build run command
		runArgs := fmt.Sprintf("-d --name %s --restart unless-stopped -p %d:%d",
			containerName, svc.Port, svc.Port)
		...
		case strings.Contains(svc.Image, "postgres"):
			...
			env[name+"_URL"] = fmt.Sprintf("postgresql://postgres:%s@127.0.0.1:%d/%s", pass, svc.Port, db)
		case strings.Contains(svc.Image, "redis"):
			env[name+"_URL"] = fmt.Sprintf("redis://127.0.0.1:%d", svc.Port)
		}
	}
